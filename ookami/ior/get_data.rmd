...
```{r}
process_ior_out <- function(pathname) {
    ...
    if(!file.exists(file.path(pathname2,"appstdout"))) {
        ...
        text <- readLines(con)
        close(con)
        if(sum(grepl("Finished",text,fixed = T))<8) {
            return(NULL)
        }
    }
    cwd <- getwd()
    ...
    system(
        "PYTHONPATH=/home/nikolays/xdmod_wsp/akrr python3 /home/nikolays/xdmod_wsp/akrr/akrr/parsers/ior_detailed_parser.py ./ ./result.xml",
...
process_many_ior_out <- function(config, pathname, id=1L) {
    results <- NULL
...
```{r}
#pathname <- "/home/nikolays/xdmod_wsp/appker_ookami/ookami/ior/Ookami-Cray/ior/2020/12/2020.12.02.04.08.23.105788"
...
#df
ior <- process_many_ior_out("Ookami, Cray","/home/nikolays/xdmod_wsp/appker_ookami/ookami/ior/Ookami-Cray/ior")

m <- ior$units=="MByte per Second"
m[is.na(m)] <- FALSE

ior$value[m] <- ior$value[m] / 1024
ior$units[m]="GB/s"
ior[m,]
...
```{r}
ior_stat <- ior %>% filter(metric_type=="statistic" & grepl("/", metric)) %>% rename(metric_org=metric)
m1 <- ior_stat$metric_org
m2 <- str_split(m1,"/", simplify=T)

ior_stat$API <- m2[,1]
ior_stat$col_ind <- m2[,2]
ior_stat$access_pattern <- m2[,3]
ior_stat$access <- m2[,4]
ior_stat$metric <- m2[,5]
ior_stat$metric_type <- NULL

ior_stat <- relocate(ior_stat,config,id,nodes,cores,API,col_ind,access_pattern,access,metric,value,units)

m_ior_stat <- ior_stat %>%
    group_by(nodes, API,col_ind,access_pattern,access,metric) %>% 
    summarise(average=mean(value),stdev=sd(value),n=n(),units=last(units))
